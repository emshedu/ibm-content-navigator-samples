...
public class SamplePluginEventHandler {
	...
	private JSONObject secretField(JSONObject configuration) {
		return configuration;//(JSONObject) ((JSONArray) configuration.get("configuration")).stream().filter(p -> ((JSONObject) p).get("name").equals("secretField")).findAny().orElse(null);
	}
